...
An example of prediction performed on one patient of the \lmttfont{TCIA-dB} with a high grade can be observed in the figure \ref{fig:Slice_hist_grad_prediction}. It has been observed that the classification confidence was often correlated with the distance to the center of the tumor, where central tumor slices where correctly classified with a higher probability than distant slices, that were either classified with a lower confidence, or even misclassified. The quality of the semantic segmentation also often guided the accuracy of the prediction, as seen in the figure \ref{fig:Slice_hist_grad_prediction}.
\begin{figure}[th!]
\centering
\includegraphics[width=0.9\linewidth]{HistologicalGradePrediction/images/Slice_hist_grad_prediction}
\caption{Example of slice-wise histological grade prediction. We globally observed that the confidence was correlated with the absolute position to the center, and that the quality of the semantic segmentation is a prerequisite to extract relevant features. Details of tumor semantic segmentation are given for 3 slices (raw image where pixels outside the predicted liver are masked in the left side, and tumor segmentation heatmap in the right side), two are correctly segmented and their histological grade was correctly predicted whereas one of them contained mis-segmented areas leading to a mis-classification of the histological grade.}
