...
%We first extracted, for each patient of the dataset, pairs of features, so that each feature in computed once for the AR and once for the PV volume.
...
%As we can see, no real differences on the accuracy of the model could be noticed when extracting features on either the expert ground truth tumor segmentation or the ROI obtained by our tumor semantic segmentation network.
%Regarding the injected phase to use, we first tested to extract features from single phase volumes separately. We realized that the accuracy of the created models was slightly better using the \ac{pv} phase volume features than the \ac{ar} features when the extraction is performed in out predicted tumor ROI. This could be explained by the fact that, in this case (i.e when features are extracted from the predicted tumor ROI), the tumor segmentation mask is shared between the AR and the PV volumes but the AR volume was registered to the PV ones, therefore, potential misregistrations would result in a drop in the relevancy of the AR features.
%Multiphase features were extracted either from our so-called \textit{perfusion} volume, or simply by stacking the features extracted from both the \ac{ar} and the \ac{pv} volumes.
%Results were better when extracting features from the \textit{perfusion} volume, potentially meaning that using a representation of the dynamic evolution could be more relevant than simply stacking features from each phase.
The best results were often obtained when combining both the AR and the PV phase volumes in our so-called \textit{perfusion} volumes (especially when using the expert defined tumor ROI as input, while results were more homogeneous when extracting features from the predicted tumor ROI), but more multiphase ``representations'' need to be investigated.
Throughout the CV training, the features picked by the model were somehow stable, and the most relevant ones were the \textit{Energy}, the \textit{Total Energy} and the \textit{High Intensity Large Area Emphasis} (HILAE) computed on the original image. These features tend to be sensitive to the presence of patterns in the images, especially the GLM one, that will respond to the presence of zones with high intensity voxels such as vessels within the tumor. Moreover, concerning the filtered images, features computed on images processed with a large filter (4mm for example) were retained more often than when using smaller filters, meaning potentially that the apparition of large structures within the tumor can be correlated with the histological grade.
