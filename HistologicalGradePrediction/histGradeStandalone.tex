...
To build an automatic histological grade prediction pipeline, we decided to extract relevant imaging features from a multiphase liver tumor semantic segmentation network. These features will then be processed by another neural network for the histological grade prediction. To build such an architecture we used 3 different datasets (2 responsible for the training of the liver tumor segmentation through a cascaded architecture and one for the training/testing of the histological grade prediction).
...
We first trained a modified version of the automatic liver segmentation network (presented in the section \todo{\textbf{X.X.X}}) using the 131 volumes of the \textbf{\lmttfont{LITS-dB}} dataset in order to segment the liver on both \ac{ar} and \ac{pv} phases independently. We then trained our \pplfont{MPF-Tumor} segmentation network using the 79 multiphase volumes of the \textbf{\lmttfont{G-dB}}. We subsequently evaluated the obtained tumor segmentation on the registered volumes of the \textbf{\lmttfont{TCIA-dB}} datasets. After extraction of the relevant imaging features, we performed a cross-validation training on the 18 multiphase volumes of the \textbf{\lmttfont{TCIA-dB}} to predict the histological grade. \\
Thereafter, we give more details about the inter-databases differences, especially those between the \textbf{\lmttfont{LITS-dB}} and the \textbf{\lmttfont{TCIA-dB}} datasets used respectively as training and testing datasets on the liver segmentation network, and those between the \textbf{\lmttfont{G-dB}} and the \textbf{\lmttfont{TCIA-dB}} datasets used respectively as training and testing datasets on the liver tumor segmentation network.

\subsection{Liver segmentation inter-dataset comparison}
...
\subsection{Liver tumor segmentation inter-dataset comparison}


We also compared both the training (\textbf{\lmttfont{G-dB}}) and the testing (\textbf{\lmttfont{TCIA-dB}}) datasets used for the liver tumor segmentation task using the same analysis as the one performed previously (see section \textcolor{red}{\textbf{ref above section}}).
...
Once the volumes of both the training (\textbf{\lmttfont{G-dB}}) and the testing (\textbf{\lmttfont{TCIA-dB}}) datasets were registered, we trained both a \pplfont{MPF-Tumor} and a \pplfont{DMP-Tumor} using the 79 volumes of the \textbf{\lmttfont{G-dB}} dataset (details regarding the architectures of the \pplfont{MPF-Tumor} and the \pplfont{DMP-Tumor} networks are given in the section \todo{add ref}).
